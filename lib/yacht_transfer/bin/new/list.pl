...
sub print_yw {
	...
	# follow link etc with WWW::Mechanize ?
	#
...
sub print_buc {
	...
	($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) =
					    localtime;
	#$buc_time = eval($mon+1) ."/0". $mday . "/" . eval($year +1900 ) . " " . eval($hour-9) .":" . $min .":" . $sec;
	#$cookie_jar->set_cookie("jordanr", 'testcookie', "$buc_time", '/', 'jordanyachtandship.com', 80, 0, 1, 60, 0);
	#$URL .= "bucwebtoolauth.htm?auth_code=jordanyacht&tool_id=2";
	#$URL .= "webtools/featured.htm"; 
