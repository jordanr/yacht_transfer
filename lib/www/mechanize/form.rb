...
module WWW
  class Mechanize
    ...
    class Form
      ...
      def inputs
	fields+radiobuttons+checkboxes
      end

      def has_input?(name)
	has_radiobutton?(name) || 
	has_checkbox?(name) || 
	has_field?(name)
      end

      def has_radiobutton?(field_name)
        ! radiobuttons.find { |f| f.name.eql? field_name }.nil?
      end

      def has_checkbox?(field_name)
        ! checkboxes.find { |f| f.name.eql? field_name }.nil?
      end
      ...
      # Fetch the first field whose name is equal to +field_name+
      def radiobutton(field_name)
        radiobuttons.find { |f| f.name.eql? field_name }
      end
      # Fetch the first field whose name is equal to +field_name+
      def checkbox(field_name)
        checkboxes.find { |f| f.name.eql? field_name }
      end
      # Fetch the first field whose name is equal to +field_name+
      def input(field_name)
        field(field_name) || checkbox(field_name) || radiobutton(field_name)
      end
...
#            self.fields.name(k.to_s).[](index).value = value
	...
	case(field)
	  when CheckBox: field.check if value
	  when RadioButton: field.check if value
      ...
      def method_missing(id,*args)
        method = id.to_s.gsub(/=$/, '')
        if input(method)
          return input(method).value if args.empty?
          return input(method).value = args[0]
        end
        super
      end
